...
function Sidebar({
  ...
  return (
    <aside className="flex h-full min-h-0 flex-col rounded-[20px] border border-emerald-400/15 bg-slate-900/85 p-4 shadow-[0_18px_45px_rgba(15,23,42,0.45)]">
      <div className="grid grid-cols-[minmax(0,1fr)_124px] items-start gap-3">
        <div className="min-w-0">
          <h1 className="text-[14px] font-semibold tracking-[0.02em] text-slate-50">{t("app.title")}</h1>
          <p className="mt-2 leading-5 text-slate-400">{t("app.description")}</p>
        </div>
      ...
      <section className={`mt-4 rounded-2xl border p-3 ${admin?.isAdmin ? "border-emerald-400/25 bg-emerald-400/10" : "border-slate-800 bg-slate-950/70"}`}>
        <p className="text-slate-400">{t("admin.title")}</p>
        <div className="mt-2 flex items-center justify-between gap-3">
          <strong className="text-[14px] font-semibold text-slate-100">{admin?.label ?? t("admin.checking")}</strong>
      ...
      <section className="mt-4 grid gap-2">
        <SummaryCard label={t("summary.total")} value={summary.total} tone="emerald" />
